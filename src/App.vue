<template>
    ...
    <div class="demo-control-bar">
      <div class="open-moda-button" @click="controlDrawerVisible = true"><a-icon><setting /></a-icon></div>
    </div>
    <a-drawer v-model:visible="controlDrawerVisible" size="400px" title="偏好设置" append-to-body destroy-on-close>
      <a-form :model="controlForm" size="small" label-width="100px" class="control-form" @submit.prevent>
        <a-form-item label="流程ID">
          <a-input v-model:value="controlForm.processId" @change="reloadProcessDesigner(true)" />
        </a-form-item>
        <a-form-item label="流程名称">
          <a-input v-model:value="controlForm.processName" @change="reloadProcessDesigner(true)" />
        </a-form-item>
        <a-form-item label="流转模拟">
          <a-switch v-model:checked="controlForm.simulation" inactive-text="停用" active-text="启用" @change="reloadProcessDesigner()" />
        </a-form-item>
        <a-form-item label="禁用双击">
          <a-switch v-model:checked="controlForm.labelEditing" inactive-text="停用" active-text="启用" @change="changeLabelEditingStatus" />
        </a-form-item>
        <a-form-item label="流程引擎">
          <a-radio-group v-model="controlForm.prefix" @change="reloadProcessDesigner()">
            <a-radio label="camunda">camunda</a-radio>
            <a-radio label="flowable">flowable</a-radio>
            <a-radio label="activiti">activiti</a-radio>
          </a-radio-group>
        </a-form-item>
        <a-form-item label="工具栏">
          <a-radio-group v-model="controlForm.headerButtonSize">
            <a-radio label="small">small</a-radio>
            <a-radio label="default">default</a-radio>
            <a-radio label="large">large</a-radio>
          </a-radio-group>
        </a-form-item>
        <a-switch v-model:checked="pageMode" active-text="dark" inactive-text="light" @change="changePageMode"></a-switch>
      </a-form>
    </a-drawer>

    <!-- <div class="demo-info-bar">
      <div class="open-model-button" @click="infoTipVisible = !infoTipVisible"><a-icon><InfoFilled /></a-icon></div>
...
export default {
  ...
  data() {
    return {
      ...
      addis: {
        CustomContentPadProvider,CustomPaletteProvider
      }
    };
  },
  created() {},
